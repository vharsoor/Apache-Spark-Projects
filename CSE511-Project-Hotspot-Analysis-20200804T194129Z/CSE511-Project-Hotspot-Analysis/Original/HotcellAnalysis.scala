...
def runHotcellAnalysis(spark: SparkSession, pointPath: String): DataFrame =
{
  ...
  val numCells = (maxX - minX + 1)*(maxY - minY + 1)*(maxZ - minZ + 1)
    ...
    var hotnessOfCellsDf = valid_cells
      .groupBy("z", "y", "x").count()
    ...
    val mean = hotnessOfCellsDf.select("cell_hotness").agg(sum("cell_hotness")).first().getLong(0).toDouble / numCells

    // calculate Standard deviation
    val stdDev = scala.math.sqrt((hotnessOfCellsDf.withColumn("sqr_cell", pow(col("cell_hotness"), 2)).select("sqr_cell").agg(sum("sqr_cell")).first().getDouble(0) / numCells) - scala.math.pow(mean, 2))
    ...
    var numOfAdjacentCells = adjHotCellNumber.withColumn("NoOfAdjCells", calculateNoOfAdjFunc(lit(minX), lit(maxX), lit(minY), lit(maxY), lit(minZ), lit(maxZ), col("x"), col("y"), col("z")))

    // user defined function to calculate G (Getis-Ord) based on the calculated information
    var gScoreFunc = udf(
      (numCells: Int , x: Int, y: Int, z: Int, numOfAdjacentCells: Int, cellNumber: Int, mean: Double, stdDev: Double)
      => HotcellUtils.calculateGScore(numCells, x, y, z, numOfAdjacentCells, cellNumber, mean, stdDev))

    // calculate G Score for every cell, order the data frame by GScore in descending order and keep only first 50 cells
    var gScoreCell = numOfAdjacentCells
      .withColumn("gScore", gScoreFunc(lit(numCells), col("x"), col("y"), col("z"), col("NoOfAdjCells"), col("cellNumber"), lit(mean), lit(stdDev)))
      .orderBy(desc("gScore")).limit(50)
    
    gScoreCell.show()

    val resultDf = gScoreCell.select(col("x"), col("y"), col("z"))
