...
object HotcellUtils {
   ...
   */
  def calculateGScore(numOfCells: Int, x: Int, y: Int, z: Int, numOfAdjacentCells: Int, cellNumber: Int , mean: Double, stdDev: Double): Double = {
     (cellNumber.toDouble - (mean * numOfAdjacentCells.toDouble)) / (stdDev
       * math.sqrt((( numOfCells.toDouble * numOfAdjacentCells.toDouble)
       - (numOfAdjacentCells.toDouble * numOfAdjacentCells.toDouble)) / (numOfCells.toDouble - 1.0)))
  }
