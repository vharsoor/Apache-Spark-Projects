...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    // Group by rectangle then order by rectangle
    // persist() is to optimize query involving long chains of transformations
    // coalesce() is to combine results from all the partitions into a single partition
